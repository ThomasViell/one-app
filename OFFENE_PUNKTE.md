...
| Datum im Vergleichsbild wandert | neu 05.08. | Gespräch | Eines der Vergleichsbilder für die Dokumentation zeigt das jeweils heutige Datum (`ProjectFormScreen.kt:484` setzt `LocalDate.now()`), und die Prüfung kennt keine Ausnahme dafür. Dadurch schlägt das Dokumentations-Tor bei jeder Veröffentlichung neu an, ohne dass sich etwas geändert hat. | Entwicklung | Entwicklung |
...
| Status des Sprach-Skripts klären | 13.07. | `publish-one-l10n.README.md` | Ungeklärt, ob das Skript im Nachbar-Repo noch verfolgt wird — siehe Befund unten. Bis zur Klärung bleibt die Datei im Repo-Root. | CEO-Entscheidung | nichts |

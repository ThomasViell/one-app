 ...
 *
 * Minimaler H.264-Encoder über die Plattform-MediaCodec. Eingabe = RGB-Bitmaps
 * (aus [com.uip.oneapp.network.internal.V4L2Camera]); Ausgabe = Annex-B-Access-
 * Units + SPS/PPS, die an [SpikeRtspServer] gereicht werden.
 *
 * Wichtig: der MS2109 zwingt VIDIOC_S_FMT auf eine ihm bekannte Auflösung (z. B.
 * 960x540 -> 800x600). Der Encoder konfiguriert sich daher LAZY aus der echten
 * Größe des ersten Bitmaps statt aus einem angenommenen Wert.
 *
 * Farbkonvertierung RGB→YUV420 über [Image]-Planes (getInputImage), damit row-/
 * pixelStride des Geräts (planar I420 vs. semi-planar NV12) korrekt bedient wird —
 * der erfahrungsgemäß heikelste Punkt.
 */
class SpikeH264Encoder(
    ...
    companion object { private const val TAG = "SpikeEncoder" }

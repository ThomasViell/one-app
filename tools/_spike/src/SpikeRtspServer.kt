 ...
 *
 * Minimaler RTSP-Server für GENAU einen H.264-Stream, RTP über TCP (interleaved,
 * RFC 2326 §10.12). TCP-Interleaving ist hier bewusst gewählt: es ist der einzige
 * Transport, der durch `adb forward` tunnelt (UDP nicht) — und vermeidet eine
 * externe RTSP-Lib.
 *
 * Pfad: rtsp://127.0.0.1:8554/cam — testbar mit ffplay/VLC (TCP erzwingen).
 */
class SpikeRtspServer(
    private val port: Int = 8554,
    private val streamPath: String = "cam",
) {
    companion object { private const val TAG = "SpikeRtsp" }
    ...
    fun start() {
        running = true
        Thread({ acceptLoop() }, "spike-rtsp-accept").apply { isDaemon = true; start() }
    }
    ...
    private fun acceptLoop() {
        try {
            ...
            while (running) {
                ...
                Thread({ s.serve() }, "spike-rtsp-session").apply { isDaemon = true; start() }
            }
        } catch (e: Exception) {
    ...
    private inner class Session(private val sock: Socket) {
                    ...
                    when (method) {
                        ...
                        "PLAY" -> {
                            respond(
                                cseq,
                                "Session: $sessionId\r\n" +
                                    "RTP-Info: url=rtsp://127.0.0.1:$port/$streamPath/streamid=0;seq=$seq;rtptime=0\r\n"
                            )
                            playing = true
        ...
        private fun handleDescribe(cseq: String) {
            ...
            respond(
                cseq,
                "Content-Base: rtsp://127.0.0.1:$port/$streamPath/\r\n",
                body = sdp
            )
        }

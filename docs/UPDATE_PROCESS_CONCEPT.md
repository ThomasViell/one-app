...
| Freigabe | — | Mensch im Portal (beta: Ersteller; stable: kein Zweit-Admin — **nicht gebaut**, Stand 06.09.2026, gemessen in `AdminReleases.razor`, Methode `Freigeben` — Stand `3c65926`; Welle `portal-freigabe-4augen` offen) |
...
| **4-Augen** | Nicht gebaut. Freigeben ist der einzige rein menschliche Akt (kein API-Endpunkt) und schreibt einen Audit-Eintrag `ReleaseApproved` (`AdminReleases.razor`, Methode `Freigeben` — Stand `3c65926`). Veröffentlichen geht auf zwei Wegen: über die Portal-Oberfläche mit Audit-Eintrag `ReleasePublished` (`AdminReleases.razor:235`), oder per API-Schlüssel über `POST releases/{id}/publish` — seit der Welle `portal-schluessel` (07.09.2026) schreibt auch dieser Weg denselben Audit-Eintrag `ReleasePublished` (`SoftwareDistributionController.cs:298-306`), Akteur als `api-key:<12-Hex-Kennung>` (SHA-256 des Schlüssels, gekürzt — der Schlüssel selbst wird nirgends gespeichert). Stand 07.09.2026. |
| **Artefakt-Tausch** | Ab **Freigabe** gesperrt, nicht erst ab Veröffentlichung: `POST releases/{id}/artifacts` antwortet danach mit 409 (`ReleaseRegeln.UploadAbgelehntGrund`, `SoftwareDistributionController.cs:224-225`) — eine Regel für API und Oberfläche gemeinsam. Vorher war der Artefaktbestand eines veröffentlichten Release mit dem Schlüssel allein austauschbar. Seit Welle `portal-schluessel`, 07.09.2026. |
| **Artefaktauswahl** | Ein Artefakt je Plattform, per Engine-Guard statt Unique-Index (`ReleaseRegeln.DoppelteplattformGrund`); im Manifest gewinnt bei mehreren Kandidaten das zuerst hochgeladene (`ReleaseRegeln.FruehestesArtefakt`, `OrderBy(CreatedAt).ThenBy(Id)`). Vorher `FirstOrDefault()` ohne Sortierung. Seit Welle `portal-schluessel`, 07.09.2026. |

...
- **W3 — Server-Stack auf der ONE (Architektur B BESTÄTIGT, Option 1 gewählt 24.06.).** W0-Befund: ONE hat null IP-Dienste (wlan0 down, kein RTSP/:12345, Kopf rein seriell) → die ONE muss zum Server werden. Unterteilt:
  - **W3a — SoftAP-Host:** `AccessPointController` (Device-Owner `SoftApConfiguration`, feste SSID `DrainQ-ONE-<Seriennr>` + WPA2-PSK, `TetheringManager.startTethering`/`stopTethering`); ggf. privilegierte Tether-Permission ins Werks-Image. STA+AP-Limit beachten. (Klein, HW per hostapd erprobt.)
  - **W3b — Steuer-/Telemetrie-Server `:12345`:** spricht dasselbe Protokoll wie der W1-Client (`OneRemoteProtocol`/`OneHardwareModels`) → Befehle vom Tablet an den seriellen Bus, Telemetrie (Meter/Sonde/Licht/Kopf) zurück. Durch W1 vorgezeichnet. **VERTRAG aus alter Slave-App `C:\Projekte\ALTE_projekte\ONE.APP` (Bominwell-ONE-SDK-Protokoll, funktionierende Vorlage):** (1) Discovery = UDP-Broadcast **:8555**; (2) Daten = TCP **:12345** JSON `SdkSendData{ miniPushInfo, videoOverlay, sendCommand:List<Int> }`; `MiniPushInfo` = distance/currentDistance, battery, electric, cameraID(Kopf), light(0-100), frequency(1=33kHz/2=640Hz/3=512Hz), btn1-6, version; ONE→Tablet pusht MiniPushInfo+OSD-Text, Tablet→ONE schickt `sendCommand`. Server = Übersetzungsschicht über `OneInternalHardwareService` (liest dieselben Werte seriell). Discovery-Handshake-Details in `ONE.APP/.../network/OneHardwareService.kt`. ONE-Server-Referenz: `ONE.APP/Android_AIO-ONE-V1.3.0-RC.1.apk` + `ONE_SDK.jar`.
  - **W3c — RTSP-Video-Server aus V4L2:** Kamera-Feed via MediaCodec (H.264) neu enkodieren + als RTSP publishen. **Long pole / Hauptrisiko (Machbarkeit + Latenz auf RK3588) → Video-Spike ZUERST.**
  - **W3d — Hardware-Sharing:** lokale ONE-UI + Netz-Server teilen EIN `/dev/ttyS5` + `/dev/video0` (Fan-out der Telemetrie/Frames, Arbitrierung der Befehle).
  - Reihenfolge-Empfehlung: W3c-Spike (De-risk) → W3a → W3b → W3c voll → W3d.
